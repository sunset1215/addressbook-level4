...
public class PersonListPanelHandle extends GuiHandle {
    ...
    public TaskListHandle navigateToPerson(String name) {
     ...
     */
    public TaskListHandle navigateToPerson(ReadOnlyTask person) {
    ...
    public TaskListHandle getPersonCardHandle(int index) {
        return getPersonCardHandle(new Task(getListView().getItems().get(index)));
    }

    public TaskListHandle getPersonCardHandle(ReadOnlyTask person) {
        Set<Node> nodes = getAllCardNodes();
        Optional<Node> personCardNode = nodes.stream()
                .filter(n -> new TaskListHandle(guiRobot, primaryStage, n).isSamePerson(person))
                .findFirst();
        if (personCardNode.isPresent()) {
            return new TaskListHandle(guiRobot, primaryStage, personCardNode.get());
        } else {
            return null;
        }
    }
