...
public class TaskListPanelHandle extends GuiHandle {
    ...
    public TaskListHandle navigateToTask(String name) {
    ...
    public TaskListHandle getTaskCardHandle(int index) {
        return getTaskCardHandle(new Task(getListView().getItems().get(index)));
    }

    public TaskListHandle getTaskCardHandle(ReadOnlyTask task) {
        Set<Node> nodes = getAllCardNodes();
        Optional<Node> taskCardNode = nodes.stream()
                .filter(n -> new TaskListHandle(guiRobot, primaryStage, n).isSameTask(task))
                .findFirst();
        if (taskCardNode.isPresent()) {
            return new TaskListHandle(guiRobot, primaryStage, taskCardNode.get());
        } else {
            return null;
        }
    }
